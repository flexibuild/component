...
namespace flexibuild\component;
 ...
 *
 * @author SeynovAM <sejnovalexey@gmail.com> copy-pasting and some corrects of source [[\yii2\base\Component]].
 *
...
trait ComponentTrait
{
    ...
    {
        if (!parent::canGetProperty($name, false)) {
            // behavior property
            $this->ensureBehaviors();
            foreach ($this->__behaviors__ as $behavior) {
    ...
    public function __call($name, $params)
    {
        if (!parent::hasMethod($name)) {
            $this->ensureBehaviors();
            foreach ($this->__behaviors__ as $object) {
    ...
    {
        if (method_exists(get_parent_class(), '__clone')) {
            parent::__clone();
        }

        $this->__events__ = [];
        $this->__behaviors__ = null;
    }
    ...
    {
        $this->ensureBehaviors();
        return !empty($this->__events__[$name]) || Event::hasHandlers($this, $name);
    }
    ...
    {
        $this->ensureBehaviors();
        if ($append || empty($this->__events__[$name])) {
            $this->__events__[$name][] = [$handler, $data];
        } else {
            array_unshift($this->__events__[$name], [$handler, $data]);
        }
    }
    ...
    {
        $this->ensureBehaviors();
        if (empty($this->__events__[$name])) {
            return false;
        }
        if ($handler === null) {
            unset($this->__events__[$name]);
            return true;
        } else {
            $removed = false;
            foreach ($this->__events__[$name] as $i => $event) {
                if ($event[0] === $handler) {
                    unset($this->__events__[$name][$i]);
                    $removed = true;
                }
            }
            if ($removed) {
                $this->__events__[$name] = array_values($this->__events__[$name]);
            }
            return $removed;
        }
    }
    ...
    public function trigger($name, Event $event = null)
    {
        ...
        if (!empty($this->__events__[$name])) {
            ...
            $event->handled = false;
            $event->name = $name;
            foreach ($this->__events__[$name] as $handler) {
    ...
    {
        $this->ensureBehaviors();
        return $this->__attachBehaviorInternal__($name, $behavior);
    }
    ...
    public function attachBehaviors($behaviors)
    {
        ...
        foreach ($behaviors as $name => $behavior) {
            $this->__attachBehaviorInternal__($name, $behavior);
        }
    }
    ...
    {
        $this->ensureBehaviors();
        if (isset($this->__behaviors__[$name])) {
            $behavior = $this->__behaviors__[$name];
            unset($this->__behaviors__[$name]);
            $behavior->detach();
            return $behavior;
        } else {
            return null;
     ...
     */
    private function __attachBehaviorInternal__($name, $behavior)
    {
        ...
        if (is_int($name)) {
            ...
            $this->__behaviors__[] = $behavior;
        } else {
            if (isset($this->__behaviors__[$name])) {
                $this->__behaviors__[$name]->detach();
            }
            $behavior->attach($this);
